...
function StickyNodes() {
  ...
  this.addWords = function (el) {
    ...
    function buildTextEls(el, shouldAdd) {
      var i, len;
      if (shouldAdd && el.nodeType === Node.TEXT_NODE && el.nodeValue.trim().length > 0) {
        textEls.push(el);
        return;
      }
...
function PlayerBall(parentNode, stickyNodes, ballOpts, sounds) {
  ...
  function drawAttached(att) {
    ...
    att.el.style.setProperty(CSS_TRANSFORM, "translate(" + x + "px," + y + "px) " + "rotate(" + th + "rad) " + "scaleX(" + Math.cos(ophi) + ") " + att.attT, null);
    return true;
  }
...
function main() {
  ...
  setTimeout(function () {
    ...
    for (i = 0, len = document.body.childNodes.length; i < len; i++) {
      ...
      window.khNodes.addTagNames(el, ["button", "canvas", "iframe", "img", "input", "select", "textarea"]);
    }
